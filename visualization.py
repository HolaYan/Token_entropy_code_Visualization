...
def analyze_repetition_logits(tokens, all_logits, tokenizer, n_gram=2, top_k=10):
    """
    检测重复并打印重复前5个token的top logits选择
    
    Args:
        tokens: 生成的token列表
        all_logits: 对应的logits列表
        tokenizer: tokenizer
        n_gram: 检测n-gram重复 (默认2-gram)
        top_k: 打印top-k logits选择
    """
    if len(tokens) < n_gram * 2:  # 至少需要能形成2个n-gram
        print("Token序列太短，无法检测重复")
        return
    
    # 检测所有重复点
    repetition_positions = []
    for i in range(len(tokens) - n_gram):
        current_ngram = tokens[i:i+n_gram]
        # 检查后续是否有相同的n-gram
        for j in range(i+n_gram, len(tokens)-n_gram+1):
            if tokens[j:j+n_gram] == current_ngram:
                repetition_positions.append(j)  # 记录所有重复位置
                print(f"🔄 发现{n_gram}-gram重复: '{' '.join(current_ngram)}' 在位置 {i} 和 {j}")
    
    if not repetition_positions:
        print(f"✅ 未发现{n_gram}-gram重复")
        return
    
    # 分析所有重复前5个token的logits
    for rep_idx, repetition_pos in enumerate(repetition_positions):
        start_pos = max(0, repetition_pos - 5)
        end_pos = repetition_pos
        
        print(f"\n📊 重复#{rep_idx+1} - 分析位置{repetition_pos}前5个token (位置 {start_pos} 到 {end_pos-1}) 的top-{top_k} logits选择:")
        print("=" * 80)
        
        for pos in range(start_pos, end_pos):
            if pos >= len(all_logits):
                continue
            ...
            print(f"\n位置 {pos}: 实际选择 '{actual_token}'")
            print("-" * 40)
            
            for rank, (prob, idx) in enumerate(zip(top_probs, top_indices)):
                candidate_token = tokenizer.decode([idx]).strip()
                is_actual = "👉" if idx == tokenizer.encode(actual_token, add_special_tokens=False)[0] else "  "
                print(f"  {rank+1:2d}. {is_actual} '{candidate_token:10s}' prob: {prob:.4f}")
